        ...
        if account and password1 and password2:
            if password1 == password2:
                ...
                if rows:
                    ...
                    cur.execute(
                        "insert into users values({:.0f}, '{}', '{}')".format(
                            time.time(), account, password1
                        )
                    )
                    conn.commit()
                conn.close()
...
class MainWindow(QMainWindow):
        ...
        self.ui.label_LocalDB_V.setText(
            str(
                datetime.datetime.fromtimestamp(
                    os.path.getmtime(os.getcwd() + "/Database/users.db")
                )
            ).split(".")[0]
        )
    ...
    def News_Search(self):
        ...
        News_result = News_Search.get_News(
            online_db_config, level, keyword, start_time, end_time
        )
        y = len(News_result)
        if y < 17:
            y = 17
        ...
        try:
            for row in range(len(News_result)):
                self.ui.tableWidget_News.setItem(
                    row, 0, QTableWidgetItem(News_result[row][1])
                )
                self.ui.tableWidget_News.setItem(
                    row, 1, QTableWidgetItem(News_result[row][2])
                )
                self.ui.tableWidget_News.setItem(
                    row, 2, QTableWidgetItem(News_result[row][0])
                )
                self.ui.tableWidget_News.setItem(
                    row, 3, QTableWidgetItem(News_result[row][3])
                )
                self.ui.tableWidget_News.setItem(
                    row, 4, QTableWidgetItem(News_result[row][4])
                )
    ...
    def export(self):  # 从服务器端更新本地数据库
        ...
        self.update_DB.status_signal.connect(
            lambda: self.ui.label_LocalDB_V.setText(
                str(
                    datetime.datetime.fromtimestamp(os.path.getmtime(DATABASE_PATH))
                ).split(".")[0]
            )
        )
    ...
    def online_db_version(self):
        ...
        for i in range(len(result_list)):  # 提取全部的时间，转换成datetime对象
            time_list.append(
                datetime.datetime.strptime(result_list[i][8], "%Y-%m-%d %H:%M:%S.%f")
            )
...
class WEB(QDialog):
    ...
    def __init__(self, parent):  # 将parent参数设置为默认值为None
        ...
        if parent:
            ...
            self.setWindowFlag(Qt.FramelessWindowHint)  # 去掉外边框
            self.setGeometry(
                parent.geometry().right() + 5, parent.geometry().top(), 630, 480
            )
