$(function(){
  // 从外部程序获取或设置key值
//        $.get('../../templates/temp.txt', function(data) {
//          var key = data;
//          // 在这里使用 key 值
//          console.log("x 的值是：" + key);
//        });
//    const key = "混凝土结构工程施工质量验收规范";  // 从外部程序获取
//    const key = "1";  // 从外部程序获取或设置key值

//   console.log()
  // 构造GET请求，将key值传递给后端
  $.get(encodeURI('/graph/'), function(result) {
    var cy =window.cy = cytoscape({
      container: document.getElementById('cy'), // 定义需要渲染的容器
      style: cytoscape.stylesheet()
        .selector('node[labels = "STANDARD"]').css({
        ...
        .selector('node[labels = "FENBU" | labels = "ZIFENBU"]').css({
